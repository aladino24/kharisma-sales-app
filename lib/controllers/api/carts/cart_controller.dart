...
class CartController extends GetxController {
  ...
  Future<void> fetchCartProduct() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        cartProductList.value = List<CartProduct>.from(jsonResult['data']
  ...
  Future<void> selectedCheckbox(String uuid, String? isSelected) async {
    print(uuid);
    print(isSelected);
    String api_cart_product =
        ApiUrl.apiUrl + 'ecom/select-cart/${uuid}/${isSelected}';
    try {
      // isLoading(true);
  ...
  Future<void> addCartProduct(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // update();
      } else if (json.decode(response.body)['code'] == 401) {
        isLoading(false);
        Get.toNamed(RoutesName.loginCustomer);
      } else {
        isLoading(false);
        closeProgressDialog();
        throw Exception(jsonDecode(response.body)['message']);
  ...
  void getTotalPrice() {
    ...
    for (var i = 0; i < cartProductList.length; i++) {
      if (cartProductList[i].isSelected == true) {
        total += int.parse(cartProductList[i].quantity!) *
            int.parse(cartProductList[i].price!.value);
      }
    }
  ...
  Future<void> updateCartProductLoading(
      String uuid, String productId, String price, int quantity) async {
    String api_cart_product = ApiUrl.apiUrl + 'ecom/cart/${uuid}';
    try {
      // showProgressDialog();
      // isLoading(true);
  ...
  Future<void> checkPrice(String? product_tmpl_id, int quantity) async {
    String api_checkprice = ApiUrl.apiUrl +
        'ecom/data-master/check-price?product_tmpl_id=${product_tmpl_id}&quantity=${quantity}';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        totalPrices[product_tmpl_id] =
            totalPrice; // Simpan totalPrice dengan ID yang sesuai
  ...
  void increment(
    ...
    if (quantity.value < int.parse(stock)) {
      ...
      Future.delayed(Duration(seconds: 3), () {
        updateCart(uuid, productId, productTmplid, productUomId, satuan,
            satuanStock, price, quantity.value);
      });
    }
    update([index]);
  }
  ...
  void selectCartProduct(int index, RxInt quantity, String uuid) {
    ...
    quantity.value = cartProductList[index].isSelected
        ? quantity.value
        : quantityGlobal.value;

    quantityGlobal.value = int.parse(cartProductList[index].quantity!);
    isAllSelected.value =
        cartProductList.every((cartProduct) => cartProduct.isSelected);
    // update();
    update(['${uuid}']);
  }
  ...
  void calculateTotalPrice() {
    ...
    for (var cartProduct in cartProductList) {
      if (cartProduct.isSelected) {
        cartProduct.price!.value = calculatePriceForCartItem(cartProduct).toString();
        int quantity = int.parse(cartProduct.quantity!);
        int price = calculatePriceForCartItem(cartProduct);
  ...
  int calculatePriceForCartItem(CartProduct cartProduct) {
    // productUom harus cuma 1 item
    var productUom = cartProduct.product?.productUom?.first;

    if (productUom != null) {
      // Berdasarkan minQuantity, ambil ProductPricelist yang paling mendekati atau kurang dari cartQuantity
      var pricelist = productUom.productPricelist?.where((priceList) {
        int minQuantity = int.parse(priceList.minQuantity ?? '0');
        int cartQuantity = int.parse(cartProduct.quantity ?? '0');
        return cartQuantity >= minQuantity;
      }).toList();

      if (pricelist != null && pricelist.isNotEmpty) {
        // Urutkan ProductPricelist berdasarkan perbedaan antara cartQuantity dan minQuantity secara mendekati
        pricelist.sort((a, b) {
          int aMinQuantity = int.parse(a.minQuantity ?? '0');
          int bMinQuantity = int.parse(b.minQuantity ?? '0');
          int cartQuantity = int.parse(cartProduct.quantity ?? '0');
          return (cartQuantity - aMinQuantity).abs().compareTo((cartQuantity - bMinQuantity).abs());
        });
        print(pricelist.first.price);
        // Ambil harga dari ProductPricelist yang paling mendekati atau kurang dari cartQuantity
        return int.parse(pricelist.first.price ?? '0');
      }
    }
    // print(cartProduct.price);
    // return price
    return int.parse(cartProduct.price!.value);
  }
